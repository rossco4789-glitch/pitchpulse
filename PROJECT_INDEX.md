...
**Setup:** `pip install kaggle`; `~/.kaggle/kaggle.json`; phone-verified account; private dataset `<user>/pitchpulse-cv-models` with `players.pt`, `pitch.pt`, `pitch_config.json` (`landmarks_m` in the 105×68 m frame, index-aligned with the pose keypoints; `exclude_landmarks` [11, 12, 19, 20] — unpainted goal-box-width points on the penalty lines, outside RANSAC consensus in 44/44 benchmark detections of #11). Diagnose rejections locally with `python tools/diagnose_benchmark_homography.py` (CPU, per-landmark residuals, leave-one-out, `--no-exclude` baseline).

**Contract:** videos over 1.5 GB are cut to 2 FPS / 720p with ffmpeg; uploads and pushes retry 5 times with exponential backoff; `vision_job.json` records DISPATCHED → RUNNING → COMPLETE | FAILED | INVALID. The worker samples exactly 2.0 FPS by timestamp and drops frames under the homography rules, each with its own counter: `too_few_landmarks` (< 6 at conf ≥ 0.5), `drop_insufficient_pitch_area` (hull < 150 m²), `drop_singular_matrix` (no/non-finite H or cond > 1e7), `drop_insufficient_inliers` (RANSAC 1.0 m keeps < 80 % or < 6), `drop_reprojection_error_high` (inlier RMSE > 0.5 m). Schema version 2. Possession-dependent moments stay null (no ball tracking); Out of Possession metrics need a settled block for ≥ 4 s, n ≥ 30 across ≥ 2 five-minute windows, and an unambiguous kit split. The collector writes `vision_metrics.json` only after the worker report, output hash, payload hash and schema all pass. Exit codes 0 / 2 setup / 3 remote failure / 4 rejected output / 5 still running.
